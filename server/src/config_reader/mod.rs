...
#[inline]
fn autorestart_parser(prog_name: &str, programm_params: &linked_hash_map::LinkedHashMap<Yaml, Yaml>) -> bool {
...
#[inline]
fn env_constructor(prog_name: &str, programm_params: &linked_hash_map::LinkedHashMap<Yaml, Yaml>, env: &mut HashMap<String, String>) {
				...
				Some(b) => {
					for (k, v) in b {
						let kk = match k {
							...
							_ => {
								eprintln!("Error parsing env key for {}. Key cnadidate: {:#?}", prog_name, k);
								continue;
							}
						};
						let vv = match v {
